...
<Callout type="info">
	...
	[`createHook()`](/docs/api-reference/workflow/create-hook) for all available
	options.
</Callout>
...
<Callout type="info">
	See the full API reference for
	[`createWebhook()`](/docs/api-reference/workflow/create-webhook) for all
	available options.
</Callout>
...
<Callout type="warning">
	When using `respondWith: "manual"`, the `respondWith()` method **must** be
	called from within a step function due to serialization requirements. This
...
async function respondToSlack(request: RequestWithResponse, text: string) {
	...
	await request.respondWith(
		new Response(JSON.stringify({ response_type: "in_channel", text }), {
			headers: { "Content-Type": "application/json" },
		})
	);
}
